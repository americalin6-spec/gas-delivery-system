...
import { authGoogleBtnStyle } from "../../lib/authStyles";
import {
  signInWithGoogle,
  subscribeToAuthCallbackMessage,
} from "../../lib/authOAuth";
...
export function GoogleSignInButton({
  ...
  useEffect(() => {
    return subscribeToAuthCallbackMessage((msg) => {
      setBusy(false);
      if (msg.status === "success") {
        finishLogin();
      } else {
        setError(authCopy.authError);
      }
    });
  }, [finishLogin]);
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      >
        <GoogleIcon />
        {busy ? authCopy.googleSigningIn : authCopy.googleSignIn}
      </button>
      {error ? (
        <p style={{ margin: "10px 0 0", color: "#fecaca", fontSize: 14 }}>{error}</p>
      ) : null}
    </div>
  );
}
